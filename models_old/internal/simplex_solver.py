...
class SimplexSolver:
    def __init__(self, linear_model: LinearModel, start_base=None):
        ...
        self.solution_fo = None

    def build_base(self, start_base=None):
        # Setups initial base
        if not start_base and not self.B_variables:
            raise ValueError("You need to specify a start base for the internal. To find automatically, use InitialBaseFinder")
    ...
    @property
    def CbT(self):
        return [item.fo_coefficient for item in self.B_variables]

    @property
    def CnT(self):
        return [item.fo_coefficient for item in self.N_variables]


    # Actual internal algorithm
    def solve(self, max_iterations=1000, __iteration__=1):
        ...
        if variable_join_N_index is None:
            ...
            self.solution_fo = np.dot(self.CbT, _xb)
